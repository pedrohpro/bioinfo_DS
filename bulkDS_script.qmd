...
ggsave("results/volcano_intgenes.png", plot = volcano_intgenes, height = 10, width = 8)
```
...
print(volcano_top20)
ggsave("results/volcano_top20.png", plot = volcano_top20, height = 10, width = 15)
```
...
kpAddCytobandLabels(kp, 
  ...
  srt = 90, 
  cex = 1, 
  col = c(rep("black", times = 6), "white", rep("black", times = 7))
)
...
degs_ord_entrez <- all_features[match(degs_ord, all_features$external_gene_name),]$entrezgene_id
...
# Network plots
BPnet <- cnetplot(ego$BP, foldChange = degs_ord_symbol_log2fc) + 
  labs(title = "GO - Biological Process") +
  theme(plot.title = element_text(hjust = 0.5))
CCnet <- cnetplot(ego$CC, foldChange = degs_ord_symbol_log2fc) + 
  labs(title = "GO - Cellular Component") +
  theme(plot.title = element_text(hjust = 0.5))
MFnet <- cnetplot(ego$MF, foldChange = degs_ord_symbol_log2fc) + 
  labs(title = "GO - Molecular Function") +
  theme(plot.title = element_text(hjust = 0.5))

# Circle plots (careful with order)
BPcirc <- cnetplot(ego$BP, foldChange = degs_ord_symbol_log2fc, circular = TRUE, colorEdge = TRUE) + 
  labs(title = "GO - Biological Process") +
  theme(plot.title = element_text(hjust = 0.5))
CCcirc <- cnetplot(ego$CC, foldChange = degs_ord_symbol_log2fc, circular = TRUE, colorEdge = TRUE) + 
  labs(title = "GO - Cellular Component") +
  theme(plot.title = element_text(hjust = 0.5))
MFcirc <- cnetplot(ego$MF, foldChange = degs_ord_symbol_log2fc, circular = TRUE, colorEdge = TRUE) + 
  labs(title = "GO - Molecular Function") +
  theme(plot.title = element_text(hjust = 0.5))
...
ggsave("results/functional/overRepGO.png", plot = overRepGO, height = 35, width = 35)
...
####   Biological process   ####
################################
#gseaplot2(ggo$BP, geneSetID = 1, title = paste0(ggo$BP@result$Description[1], " - padj = ", signif(ggo$BP@result$p.adjust[1], digits=2)))
# gsea plot
gsea_plot <- gseaplot2(
  ggo$BP, 
  geneSetID = 1:5, 
...
ggsave("results/BPgsea.png", plot = gsea_plot, height = 6, width = 9)

# dot plot
dot_plot <- dotplot(ggo$BP, showCategory=30, title = "dot plot") #what is the ranking system here???? (não achei em nenhuma das colunas)
ggsave("results/BPdotplot.png", plot = dot_plot, height = 10, width = 5) # e onde ta o gene ratio???
...
ggsave("results/CCgsea.png", plot = gsea_plot, height = 6, width = 9)

# dot plot
dot_plot <- dotplot(ggo$CC, showCategory=30, title = "dot plot")
ggsave("results/CCdotplot.png", plot = dot_plot, height = 10, width = 5)
...
ggsave("results/MFgsea.png", plot = gsea_plot, height = 9, width = 15)

# dot plot
dot_plot <- dotplot(ggo$MF, showCategory=30, title = "dot plot")
ggsave("results/MFdotplot.png", plot = dot_plot, height = 10, width = 5)
