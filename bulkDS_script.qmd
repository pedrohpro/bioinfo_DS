...
!!!!!!!!!!! Incluir asterisco de signifcativo pra cada par padj \< 0.05

```{r}
# Long format for genes of interest
norm_int <- norm_df[rownames(norm_df) %in% intGenes,]
local_res <- 
rownames(norm_int)[] <- paste0(rownames(norm_int)[], " *") # adding sig asterisk
...
```{r}
# mudar background do título de cinza pra outra cor
# colocar asterisco de significância
ggplot(long_int, aes(x = gene, y = expression, fill = condition)) +
...
pheatmap(
  ...
  gaps_col = 12,
  scale = "row", # Z-score
  annotation_colors = list(log2FoldChange = c('blue', 'white', 'red'), significance = c(sig = 'lightgreen', notSig = 'white')) # set log2fc 0 to white
)
...
kable(int_df[order(int_df$padj),], format = "html", caption = "Genes of interest") %>%
  kable_styling()
